...
archivo <- file.path("Muestra de tickets.xlsx")
...
# =====================================================
# Preparación de la base para análisis y optimización
# =====================================================

# Se conserva una base completa con todas las columnas originales.
# Esta base será necesaria posteriormente para construir el problema
# de optimización en Python.

df_completo <- df_raw %>%
  mutate(
    id_fila_original = row_number()
  ) %>%
  filter(
    !is.na(`Worked hours`),
    `Worked hours` > 0
...
# Base numérica utilizada en PCA, PLS, MLR y Random Forest
df <- df_completo %>%
  select(all_of(vars_modelo))

cat(
  "\nFilas tras eliminar Worked hours faltantes o iguales a 0:",
  nrow(df),
  "\n"
)

cat(
  "Columnas conservadas en la base completa:",
  ncol(df_completo),
  "\n"
)
...
# =====================================================
# Construcción de las bases limpias
# =====================================================

# Base numérica limpia utilizada por los modelos de R
df_limpio <- df[idx, , drop = FALSE]

# Base completa limpia.
# Se aplican exactamente los mismos índices obtenidos durante
# la limpieza mediante PCA, SPE y Hotelling T².
df_limpio_completo <- df_completo[idx, , drop = FALSE]

cat(
  "\nDataset numérico limpio:",
  nrow(df_limpio),
  "observaciones\n"
)
...
cat(
  ...
  nrow(df_limpio_completo),
  "observaciones y",
  ncol(df_limpio_completo),
  "columnas\n"
)

cat(
  "Observaciones eliminadas:",
  nrow(df) - nrow(df_limpio),
  "\n"
)

# Comprobación de consistencia
stopifnot(nrow(df_limpio) == nrow(df_limpio_completo))
stopifnot(
  all(
    df_limpio$`Worked hours` ==
      df_limpio_completo$`Worked hours`
  )
)
```
...
```{r}
# =====================================================
# PCA exploratorio incluyendo Worked hours
# =====================================================

vars_pca <- c("Priority level",
              "Inward Tickets",
              "Outward Tickets",
              "Inward Project",
              "Outward Project",
...
# Matriz de datos
X_pca <- df_limpio[, vars_pca]

# Centrado y escalado
X_pca_scaled <- scale(X_pca)

# PCA
pca_mod <- prcomp(X_pca_scaled, center = FALSE, scale. = FALSE)

# Loadings
loadings <- as.data.frame(pca_mod$rotation)
loadings$Variable <- rownames(loadings)

# Función para loading plots
plot_loading <- function(data, pc_x, pc_y, titulo) {
  
  x <- paste0("PC", pc_x)
  y <- paste0("PC", pc_y)
  
  plot(data[[x]], data[[y]],
       xlab = paste0(pc_x, " PCA component"),
       ylab = paste0(pc_y, " PCA component"),
       main = titulo,
       pch = 19,
       col = "blue",
       xlim = c(-1, 1),
...
# PC1 vs PC2
plot_loading(loadings, 1, 2, "PCA Loading Plot: PC1 vs PC2")

# PC3 vs PC4
plot_loading(loadings, 3, 4, "PCA Loading Plot: PC3 vs PC4")
```

```{r}
# =====================================================
# Rotación Varimax sobre los loadings del PCA
# =====================================================

# Número de componentes a rotar
A_rot <- 4

# Loadings originales
P <- pca_mod$rotation[, 1:A_rot]

# Rotación Varimax
varimax_mod <- varimax(P)

# Loadings rotados
loadings_rot <- as.data.frame(varimax_mod$loadings[, 1:A_rot])
colnames(loadings_rot) <- paste0("RC", 1:A_rot)
loadings_rot$Variable <- rownames(loadings_rot)

# Función para loading plots rotados
plot_loading_rot <- function(data, comp_x, comp_y, titulo) {
  
  x <- paste0("RC", comp_x)
  y <- paste0("RC", comp_y)
  
  plot(data[[x]], data[[y]],
       xlab = paste0("Rotated component ", comp_x),
       ylab = paste0("Rotated component ", comp_y),
       main = titulo,
       pch = 19,
       col = "blue",
       xlim = c(-1, 1),
...
# Loading plots rotados
plot_loading_rot(loadings_rot, 1, 2,
                 "Varimax Rotated Loading Plot: RC1 vs RC2")

plot_loading_rot(loadings_rot, 3, 4,
                 "Varimax Rotated Loading Plot: RC3 vs RC4")
```

```{r}
# =====================================================
# PCA score plots ROTADOS coloreados por Priority level
# =====================================================

# Número de componentes rotadas
A_rot <- 4

# Loadings originales del PCA
P <- pca_mod$rotation[, 1:A_rot]

# Rotación Varimax
varimax_mod <- varimax(P)

# Scores originales del PCA
T_pca <- as.matrix(pca_mod$x[, 1:A_rot])

# Scores rotados
scores_rot <- as.data.frame(T_pca %*% varimax_mod$rotmat)

# Renombrar componentes rotadas
colnames(scores_rot) <- paste0("RC", 1:A_rot)

# Añadir variable Priority level
scores_rot$Priority_level <- as.factor(df_limpio$`Priority level`)

# Colores discretos por prioridad
colores_priority <- c(
  "1" = "red",
  "2" = "orange",
  "3" = "green",
...
# Función para score plots rotados
plot_scores_rot <- function(data, comp_x, comp_y, titulo) {
  
  x <- paste0("RC", comp_x)
  y <- paste0("RC", comp_y)
  
  z <- data$Priority_level
  ...
  plot(data[[x]], data[[y]],
       ...
       main = titulo,
       pch = 21,
       bg = colores_priority[as.character(z)],
       col = "gray30",
       xlim = c(-20, 20),
       ylim = c(-20, 20))
  ...
  legend("right",
         legend = names(colores_priority),
         pt.bg = colores_priority,
         pch = 21,
...
# RC1 vs RC2
plot_scores_rot(scores_rot, 1, 2,
                "Varimax Rotated Score Plot: RC1 vs RC2")

# RC3 vs RC4
plot_scores_rot(scores_rot, 3, 4,
                "Varimax Rotated Score Plot: RC3 vs RC4")
```
...
png(file.path("g11_roc_plsda.png"),
...
ggsave(file.path("g8_rmse_cv_plsda.png"),
       g8,  width = 7, height = 5, dpi = 150)
ggsave(file.path("g9_score_plsda.png"),
       g9,  width = 7, height = 6, dpi = 150)
ggsave(file.path("g10_wc_plsda.png"),
       g10, width = 7, height = 6, dpi = 150)
ggsave(file.path("g12_permutation_test.png"),
       g12, width = 8, height = 5, dpi = 150)
ggsave(file.path("g13_confusion_plsda.png"),
...
ggsave(file.path("g1_rmse_cv_pls_pcr.png"),
       g1, width = 8, height = 5, dpi = 150)
ggsave(file.path("g2_vip_pls.png"),
       g2, width = 7, height = 5, dpi = 150)
ggsave(file.path("g3_wstar_pls.png"),
       g3, width = 7, height = 6, dpi = 150)
ggsave(file.path("g4_t_vs_u_pls.png"),
...
pred_rf <- predict(rf_mod, newdata = as.matrix(X_full[idx_test, ]))
...
ggsave(file.path("g7_rf_vs_vip.png"),
...
ggsave(file.path("g6_comparacion_modelos.png"),
       grid.arrange(g6a, g6b, ncol = 2),
       width = 12, height = 5, dpi = 150)
ggsave(file.path("g14_residuos.png"),
       g14, width = 8, height = 5, dpi = 150)

png(file.path("g5_obs_vs_pred.png"),
...
```{r}
# =====================================================
# EXPORTACIÓN PARA LA FASE DE OPTIMIZACIÓN EN PYTHON
# =====================================================

# Crear carpeta de salida si todavía no existe
dir.create(
  "datos_optimizacion",
  showWarnings = FALSE,
  recursive = TRUE
...
print(names(df_limpio_completo))

# =====================================================
# Selección de variables para optimización
# =====================================================

columnas_optimizacion_deseadas <- c(
  "id_fila_original",
  "Issue key",
  "Assignee",
  "Team",
...
# Seleccionar únicamente las columnas que realmente existan
columnas_optimizacion_existentes <- intersect(
  columnas_optimizacion_deseadas,
  names(df_limpio_completo)
)

# Informar de columnas disponibles y ausentes
columnas_ausentes <- setdiff(
  columnas_optimizacion_deseadas,
  names(df_limpio_completo)
)

cat("\nColumnas seleccionadas para optimización:\n")
print(columnas_optimizacion_existentes)

if (length(columnas_ausentes) > 0) {
  cat("\nColumnas deseadas que no existen en la base:\n")
  print(columnas_ausentes)
}

df_optimizacion <- df_limpio_completo %>%
  select(all_of(columnas_optimizacion_existentes))

write.csv(
  df_optimizacion,
  file = file.path(
    "datos_optimizacion",
    "tickets_para_optimizacion.csv"
  ),
  row.names = FALSE,
  na = ""
)

# Exportar toda la base limpia.
# Esta es la opción recomendada porque evita perder información
# que pueda resultar útil en fases posteriores.
write.csv(
  df_limpio_completo,
  file = file.path(
    "datos_optimizacion",
    "tickets_limpios_completos.csv"
  ),
  row.names = FALSE,
  na = ""
)

# Exportar también la base numérica exacta utilizada en los modelos
write.csv(
  df_limpio,
  file = file.path(
    "datos_optimizacion",
    "tickets_limpios_modelado.csv"
  ),
  row.names = FALSE,
  na = ""
)
...
cat(
  "\nArchivos exportados correctamente en la carpeta:",
  normalizePath("datos_optimizacion"),
  "\n"
)
```
